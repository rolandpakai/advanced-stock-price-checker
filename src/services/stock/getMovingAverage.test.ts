...
describe("getMovingAverage", () => {
  ...
  it("should round result to 2 decimal places", async () => {
    ...
    const result = await getMovingAverage("AAPL", 2);

    expect(result).toBe(150.5);
    expect(mockGetLastNStockQuote).toHaveBeenCalledWith("AAPL", 2);
  });
  ...
  it("should handle different stock symbols correctly", async () => {
    const mockQuotes = [
      {
        id: 1,
        symbol: "GOOGL",
    ...
    const result = await getMovingAverage("GOOGL", 2);

    expect(result).toBe(2550);
    expect(mockGetLastNStockQuote).toHaveBeenCalledWith("GOOGL", 2);
  });

  it("should handle repository errors", async () => {
    mockGetLastNStockQuote.mockRejectedValue(new Error("Database error"));

    await expect(getMovingAverage("AAPL", 10)).rejects.toThrow("Database error");
    expect(mockGetLastNStockQuote).toHaveBeenCalledWith("AAPL", 10);
  });
